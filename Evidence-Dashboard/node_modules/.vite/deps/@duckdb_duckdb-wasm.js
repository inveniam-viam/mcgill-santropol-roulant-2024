...
var E = class {
  ...
  async send(e) {
    ...
    let t = new p(this._bindings, this._conn, r), o = await RecordBatchReader.from(t);
...
function R(s) {
  switch (s.typeId) {
    ...
    case Type.List:
      return { sqlType: "list", valueType: R(s.valueType) };
    ...
    case Type.Utf8:
      return { sqlType: "utf8" };
    case Type.Struct:
      return { sqlType: "struct", fields: s.children.map((r) => _(r.name, r.type)) };
    case Type.Map: {
      let e = s;
      return { sqlType: "map", keyType: R(e.keyType), valueType: R(e.valueType) };
    }
    case Type.Time:
  ...
  async onMessage(e) {
    ...
    try {
      switch (e.type) {
        ...
        case "START_PENDING_QUERY": {
          let r = this._bindings.startPendingQuery(e.data[0], e.data[1]), t = [];
        ...
        case "FETCH_QUERY_RESULTS": {
          ...
          this.postMessage({ messageId: this._nextMessageId++, requestId: e.messageId, type: "QUERY_RESULT_CHUNK", data: r }, [r.buffer]);
          break;
        }
        case "GET_TABLE_NAMES": {
...
var C = async () => WebAssembly.validate(new Uint8Array([0, 97, 115, 109, 1, 0, 0, 0, 1, 5, 1, 96, 0, 1, 123, 3, 2, 1, 0, 10, 10, 1, 8, 0, 65, 0, 253, 15, 253, 98, 11]));
var U = () => (async (s) => {
  try {
...
var B = () => M() ? "node" : navigator.userAgent;
var Me = () => B().includes("Firefox");
var Be = () => /^((?!chrome|android).)*safari/i.test(B());
function Ge() {
...
var h = null;
async function de() {
  return k == null && (k = typeof BigInt64Array < "u"), y == null && (y = await f()), g == null && (g = await U()), S == null && (S = await C()), h == null && (h = await F()), { bigInt64Array: k, crossOriginIsolated: M() || globalThis.crossOriginIsolated || false, wasmExceptions: y, wasmSIMD: S, wasmThreads: g, wasmBulkMemory: h };
}
async function xe(s) {
  let e = await de();
  if (e.wasmExceptions) {
...
export {
  ...
  P as ConsoleLogger,
  Z as DuckDBAccessMode,
  q as DuckDBDataProtocol,
  te as LogEvent,
  ee as LogLevel,
  se as LogOrigin,
  re as LogTopic,
  W as PACKAGE_NAME,
  v as PACKAGE_VERSION,
  fe as PACKAGE_VERSION_MAJOR,
  Ce as PACKAGE_VERSION_MINOR,
  Ue as PACKAGE_VERSION_PATCH,
  ne as StatusCode,
  X as TokenType,
  A as VoidLogger,
  L as WorkerRequestType,
  D as WorkerResponseType,
  a as WorkerTask,
  Ye as createWorker,
  Ge as getJsDelivrBundles,
  Re as getLogEventLabel,
  pe as getLogLevelLabel,
  me as getLogOriginLabel,
  _e as getLogTopicLabel,
  de as getPlatformFeatures,
  Me as isFirefox,
  M as isNode,
  Be as isSafari,
  xe as selectBundle
