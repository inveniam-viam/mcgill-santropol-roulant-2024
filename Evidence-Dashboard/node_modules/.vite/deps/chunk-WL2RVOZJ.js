...
function tableFromJSON(array) {
  ...
  const batch = new RecordBatch(new Schema2(vector.type.children), vector.data[0]);
  ...
  return function(source) {
    return __asyncGenerator(this, arguments, function* () {
      ...
      try {
        ...
        try {
          if (!_d2 && !_a5 && (_b2 = source_1.return))
            yield __await(_b2.call(source_1));
        } finally {
          if (e_1)
            throw e_1.error;
        }
...
var JSONTypeAssembler = class extends Visitor {
  ...
  visitNull({ typeId }) {
    return { "name": Type2[typeId].toLowerCase() };
  }
  visitInt({ typeId, bitWidth, isSigned }) {
    return { "name": Type2[typeId].toLowerCase(), "bitWidth": bitWidth, "isSigned": isSigned };
  }
  visitFloat({ typeId, precision }) {
    return { "name": Type2[typeId].toLowerCase(), "precision": Precision[precision] };
  }
  visitBinary({ typeId }) {
    return { "name": Type2[typeId].toLowerCase() };
  }
  ...
  visitDecimal({ typeId, scale, precision, bitWidth }) {
    return { "name": Type2[typeId].toLowerCase(), "scale": scale, "precision": precision, "bitWidth": bitWidth };
  }
  visitDate({ typeId, unit }) {
    return { "name": Type2[typeId].toLowerCase(), "unit": DateUnit[unit] };
  }
  visitTime({ typeId, unit, bitWidth }) {
    return { "name": Type2[typeId].toLowerCase(), "unit": TimeUnit[unit], bitWidth };
  }
  visitTimestamp({ typeId, timezone, unit }) {
    return { "name": Type2[typeId].toLowerCase(), "unit": TimeUnit[unit], timezone };
  }
  visitInterval({ typeId, unit }) {
    return { "name": Type2[typeId].toLowerCase(), "unit": IntervalUnit[unit] };
  }
  visitList({ typeId }) {
    return { "name": Type2[typeId].toLowerCase() };
  }
  ...
  visitFixedSizeBinary({ typeId, byteWidth }) {
    return { "name": Type2[typeId].toLowerCase(), "byteWidth": byteWidth };
  }
  visitFixedSizeList({ typeId, listSize }) {
    return { "name": Type2[typeId].toLowerCase(), "listSize": listSize };
  }
  visitMap({ typeId, keysSorted }) {
    return { "name": Type2[typeId].toLowerCase(), "keysSorted": keysSorted };
  }
};
...
var JSONVectorAssembler = class _JSONVectorAssembler extends Visitor {
  ...
  static assemble(...batches) {
    const assemlber = new _JSONVectorAssembler();
  ...
  visit({ name }, data) {
    ...
    return Object.assign({ "name": name, "count": length, "VALIDITY": DataType.isNull(type) ? void 0 : nullCount <= 0 ? Array.from({ length }, () => 1) : [...new BitIterator(nullBitmap, offset, length, null, getBit)] }, super.visit(data.clone(type, offset, length, 0, buffers)));
  }
  visitNull() {
    return {};
  }
...
export {
  ...
  tableFromArrays,
  RecordBatch,
  MessageHeader2 as MessageHeader,
  Message2 as Message,
  MessageReader,
  AsyncMessageReader,
  JSONMessageReader,
...
//# sourceMappingURL=chunk-WL2RVOZJ.js.map
